...
@allure.feature('登录模块')
class TestLogin:
    # @pytest.mark.parametrize('title,bodyData,expData', get_excel_data('登录模块', 'Login', '标题', '请求参数', '响应预期结果'))
    @pytest.mark.parametrize('title,bodyData,expData', get_yaml_caseData(os.path.join(testData_path, 'loginCase.yml')))
    @allure.title("{title}")
